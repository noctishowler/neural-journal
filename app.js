...
function swipe(direction) {
  ...
  // Lock screen:
  // every direction belongs to the password.
  if (view === 'lock') {
...
function showDate() {
  ...
  for (
    ...
    for (
      ...
      // Calendar buttons themselves do not choose dates.
      // Pointer drag controls selection so stale focus cannot
      // accidentally replace selectedDate.
      const dayButton =
        button(
          String(
            date.getDate()
...
document.addEventListener(
  ...
  event => {
     ...
     *
     * A deliberate horizontal LEFT swipe still means Back.
     * Otherwise releasing after calendar selection opens
     * the highlighted date.
     */
    if (
      wasCalendarPointer
    ) {
      const deliberateLeftSwipe =
        distance >= 45 &&
        elapsed < 700 &&
        Math.abs(dx) >
          Math.abs(dy) &&
